...
public class MaterialSearchViewController implements ViewMode.ModeChangeListener,
    ...
    public void onViewModeChanged(int newMode) {
        if(Utils.enableExecuteLocalSearch(mActivity.getActivityContext())){
            return;
        }
    ...
    public void showSearchActionBar(int state, boolean animate) {
        ...
        final boolean onlyActionBar = state == SEARCH_VIEW_STATE_ONLY_ACTIONBAR &&
                (Utils.enableExecuteLocalSearch(mActivity.getActivityContext()) ?
                        true : mController.shouldShowSearchBarByDefault(mViewMode));
        ...
        final int filterVisibility = Utils.enableExecuteLocalSearch(mActivity.getActivityContext())
                && onlyActionBar ? View.VISIBLE : View.GONE;
        if (animate) {
    ...
    public void changeFactorAction(String factor, boolean isUser) {
        ...
        mController.executeLocalSearch(mKeyWord, factor, isUser);
    }
    ...
    public void restoreLocalSearch(Context context) {
        if (!Utils.enableExecuteLocalSearch(context)) {
            return;
        }
        if (mControllerState == SEARCH_VIEW_STATE_VISIBLE) {
            showSearchActionBar(SEARCH_VIEW_STATE_VISIBLE);
                ...
                && mKeyWord != null && mFactorId != INIT_ID) {
        ...
        } else if (mControllerState == SEARCH_VIEW_STATE_GONE
                && mKeyWord != null && mFactorId != INIT_ID) {
            mSearchActionView.setQueryText(mKeyWord);
            mSearchFactorView
                    .changeCheckRadioButton(mSearchFactorView.getFactor(mFactorId), false);
    ...
    public static final boolean isOnlyActionbar(MaterialSearchViewController
            searchViewController) {
        if (searchViewController == null) {
            return false;
        }
        return searchViewController.mControllerState == SEARCH_VIEW_STATE_ONLY_ACTIONBAR;
    }
